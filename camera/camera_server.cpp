...
int detectDot() {
  ...
  tTimer = millis();
  int maxIndex =
      detectObject(rgb, ARRAY_LENGTH, IMAGE_WIDTH, hueDesired, hueThreshold,
                   satThreshold, valueThreshold, pixelThreshold);
  Serial.println("Detection took " + String(millis() - tTimer) + " ms");

  if (maxIndex == ERROR_NOTHING_DETECTED) {
    Serial.println("Not enough pixels detected");
    esp_camera_fb_return(fb);  // camera frame buffer
    heap_caps_free(ptrVal);    // rgb data
...
// int detectDotWithFrame(camera_fb_t *fb) {
//   // make sure psram is available
//   if (!psramFound()) {
//     Serial.println("error: no psram available to store the RGB data");
//     return ERROR_CATASTROPHIC;
...
//   int PIXEL_COUNT = IMAGE_WIDTH * IMAGE_HEIGHT;
//   int ARRAY_LENGTH =
//       PIXEL_COUNT * 3;  // calculate memory required to store the RGB data
//       (i.e.
//                         // number of pixels in the jpg image x 3)
...
//       MALLOC_CAP_SPIRAM);  // allocate memory space for the rgb data
//   uint8_t *rgb = (uint8_t *)
//       ptrVal;  // create the 'rgb' array pointer to the allocated memory
//       space
//   // Serial.println("Free psram after rgb data allocated = " +
//   //                String(heap_caps_get_free_size(MALLOC_CAP_SPIRAM) / 1024)
//   +
