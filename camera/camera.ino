/*******************************************************************************************************************
 *
 *                         ESP32Cam development board demo sketch using Arduino
 *IDE or PlatformIO Github: https://github.com/alanesq/ESP32Cam-demo
 *
 *                                     Tested with ESP32 board manager
 *version  2.0.11
 *
 *     Starting point sketch for projects using the esp32cam development board
 *with the following features web server with live video streaming and RGB data
 *from camera demonstrated. sd card support using 1-bit mode (data pins are
 *usually 2,4,12&13 but using 1bit mode only uses pin 2) flash led is still
 *available for use (pin 4) and does not flash when accessing sd card Stores
 *image in Spiffs if no sd card present PWM control of the illumination/flash
 *LED
 *
 *     GPIO:
 *        You can use io pins 13 and 12 for input or output (but 12 must not be
 *high at boot) You could also use pins 1 & 3 if you do not use Serial (disable
 *serialDebug in the settings below) Pins 14, 2 & 15 should be ok to use if you
 *are not using an SD Card More info:
 *https://randomnerdtutorials.com/esp32-cam-ai-thinker-pinout/
 *
 *     You can use a MCP23017 io expander chip to give 16 gpio lines by enabling
 *'useMCP23017' in the setup section and connecting the i2c pins to 12 and 13 on
 *the esp32cam module.  Note: this requires the adafruit MCP23017 library to be
 *installed.
 *
 *     Created using the Arduino IDE with ESP32 module installed, no additional
 *libraries required ESP32 support for Arduino IDE:
 *https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_index.json
 *
 *     Info on the esp32cam board:
 *https://randomnerdtutorials.com/esp32-cam-video-streaming-face-recognition-arduino-ide/
 *                                  https://github.com/espressif/esp32-camera
 *
 *     To see a more advanced sketch along the same format as this one have a
 *look at https://github.com/alanesq/CameraWifiMotion which includes email
 *support, FTP, OTA updates and motion detection
 *
 *     esp32cam-demo is distributed in the hope that it will be useful, but
...
bool sendRGBfile = 0;  // if set '/rgb' will just return raw rgb data which can
                       // be saved as a file rather than display a HTML pag

uint16_t dataRefresh =
    2;  // how often to refresh data on root web page (seconds)
uint16_t imagerefresh =
    2;  // how often to refresh the image on root web page (seconds)

const bool serialDebug = 1;  // show debug info. on serial port (1=enabled,
                             // disable if using pins 1 and 3 as gpio)

#define useMCP23017 \
  0  // if MCP23017 IO expander chip is being used (on pins 12 and 13)
...
// set camera resolution etc. in 'initialiseCamera()' and
// 'cameraImageSettings()'
...
// NTP - Internet time - see -
// https://randomnerdtutorials.com/esp32-ntp-timezones-daylight-saving/
#include "time.h"
struct tm timeinfo;
const char *ntpServer = "pool.ntp.org";
const char *TZ_INFO =
    "GMT+0BST-1,M3.5.0/01:00:00,M10.5.0/02:00:00";  // enter your time zone
...
String spiffsFilename =
    "/image.jpg";                    // image name to use when storing in spiffs
...
void setup() {
  ...
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG,
                 0);  // Turn-off the 'brownout detector'
  ...
  server.on("/data",
            handleData);  // suplies data to periodically update root (AJAX)
  ...
  if (!SPIFFS.begin(true)) {
    if (serialDebug)
      Serial.println(
          ("An Error has occurred while mounting SPIFFS - restarting"));
    delay(5000);
  ...
  if (!SD_MMC.begin("/sdcard", true)) {  // if loading sd card fails
    // note: ('/sdcard", true)' = 1bit mode - see:
    ...
    if (cardType == CARD_NONE) {  // if invalid card found
      ...
      uint16_t SDfreeSpace =
          (uint64_t)(SD_MMC.totalBytes() - SD_MMC.usedBytes()) / (1024 * 1024);
      if (serialDebug)
  ...
  // discover the number of image files already stored in '/img' folder of the
  // sd card and set image file counter accordingly
  imageCounter = 0;
  if (sdcardPresent) {
    int tq = fs.mkdir("/img");  // create the '/img' folder on sd card (in case
...
bool initialiseCamera() {
  ...
  if (!psramFound()) {
    if (serialDebug)
      Serial.println(
          "Warning: No PSRam found so defaulting to image size 'CIF'");
...
//            interesting info on exposure times here:
...
//    // If you enable gain_ctrl or exposure_ctrl it will prevent a lot of the
//    other settings having any effect
//    // more info on settings here:
//    https://randomnerdtutorials.com/esp32-cam-ov2640-camera-settings/
//    s->set_gain_ctrl(s, 0);                       // auto gain off (1 or 0)
//    s->set_exposure_ctrl(s, 0);                   // auto exposure off (1 or
//    0) s->set_agc_gain(s, cameraImageGain);          // set gain manually (0 -
//    30) s->set_aec_value(s, cameraImageExposure);     // set exposure manually
//    (0-1200) s->set_vflip(s, cameraImageInvert);           // Invert image (0
//    or 1) s->set_quality(s, 10);                        // (0 - 63)
//    s->set_gainceiling(s, GAINCEILING_32X);       // Image gain
//    (GAINCEILING_x2, x4, x8, x16, x32, x64 or x128) s->set_brightness(s,
//    cameraImageBrightness);  // (-2 to 2) - set brightness s->set_lenc(s, 1);
//    // lens correction? (1 or 0) s->set_saturation(s, 0); // (-2 to 2)
//    s->set_contrast(s, cameraImageContrast);      // (-2 to 2)
//    s->set_sharpness(s, 0);                       // (-2 to 2)
//    s->set_hmirror(s, 0);                         // (0 or 1) flip
//    horizontally s->set_colorbar(s, 0);                        // (0 or 1) -
//    show a testcard s->set_special_effect(s, 0);                  // (0 to 6?)
//    apply special effect s->set_whitebal(s, 0);                        //
//    white balance enable (0 or 1) s->set_awb_gain(s, 1); // Auto White Balance
//    enable (0 or 1) s->set_wb_mode(s, 0);                         // 0 to 4 -
//    if awb_gain enabled (0 - Auto, 1 - Sunny, 2 - Cloudy, 3 - Office, 4 -
//    Home) s->set_dcw(s, 0);                             // downsize enable? (1
//    or 0)? s->set_raw_gma(s, 1);                         // (1 or 0)
//    s->set_aec2(s, 0);                            // automatic exposure
//    sensor?  (0 or 1) s->set_ae_level(s, 0);                        // auto
//    exposure levels (-2 to 2) s->set_bpc(s, 0);                             //
//    black pixel correction s->set_wpc(s, 0);                             //
...
// ----------------------------------------------------------------
// note: I am not sure PWM is very reliable on the esp32cam - requires more
// testing
void setupFlashPWM() {
...
void sendText(WiFiClient &client, String theText) {
  ...
  if (serialDebug || theText.indexOf("error") > 0)
    Serial.println(theText);  // if text contains "error"
}
...
void changeResolution(framesize_t tRes = FRAMESIZE_96X96) {
  ...
  delay(50);
  if (tRes ==
      FRAMESIZE_96X96) {  // taken as none supplied so cycle through several
    if (FRAME_SIZE_IMAGE == FRAMESIZE_QVGA)
      tRes = FRAMESIZE_VGA;
    else if (FRAME_SIZE_IMAGE == FRAMESIZE_VGA)
      tRes = FRAMESIZE_SVGA;
    else if (FRAME_SIZE_IMAGE == FRAMESIZE_SVGA)
      tRes = FRAMESIZE_SXGA;
    else
      tRes = FRAMESIZE_QVGA;
  }
...
byte storeImage() {
  ...
  if (flashRequired) {
    delay(100);
    brightLed(
        currentBrightness);  // change LED brightness back to previous state
  }
  if (!fb) {
  ...
  if (!sdcardPresent) {
    ...
    if (!file) {
      if (serialDebug)
        Serial.println(
            "Failed to create file in Spiffs - will format and try again");
      if (!SPIFFS.format()) {  // format spiffs
        if (serialDebug) Serial.println("Spiffs format failed");
      } else {
        file = SPIFFS.open(spiffsFilename,
                           FILE_WRITE);  // try again to create new file
        if (!file) {
          if (serialDebug)
  ...
  if (sdcardPresent) {
    ...
    String SDfilename = "/img/" + String(imageCounter + 1) +
                        ".jpg";                   // build the image file name
    ...
    if (!file) {
      ...
      if (file.write(fb->buf,
                     fb->len)) {  // File created ok so save image to it
...
void rootUserInput(WiFiClient &client) {
  ...
  //        Note:  if using an input box etc. you would read the value with the
  //        command:    String Bvalue = server.arg("demobutton1");
...
void handleRoot() {
  ...
  client.write("<FORM action='/' method='post'>\n");  // used by the buttons in
                                                      // the html (action = the
                                                      // web page to send it to
  ...
  //     Info on Javascript (can be inserted in to the HTML):
  //     https://www.w3schools.com/js/default.asp
  //                               Verify your HTML is valid:
  //                               https://validator.w3.org/

  // ---------------------------------------------------------------------------------------------
  //  info which is periodically updated using AJAX -
  //  https://www.w3schools.com/xml/ajax_intro.asp

  // empty lines which are populated via vbscript with live data from
  // http://x.x.x.x/data in the form of comma separated text
  int noLines = 5;  // number of text lines to be populated by javascript
  for (int i = 0; i < noLines; i++) {
    client.println("<span id='uline" + String(i) + "'></span><br>");
  }

  // Javascript - to periodically update the above info lines from
  // http://x.x.x.x/data Note: You can compact the javascript to save flash
  // memory via https://www.textfixer.com/html/compress-html-compression.php
  //       The below = client.printf(R"=====(  <script> function getData() { var
  //       xhttp = new XMLHttpRequest(); xhttp.onreadystatechange = function() {
  //       if (this.readyState == 4 && this.status == 200) { var receivedArr =
  //       this.responseText.split(','); for (let i = 0; i < receivedArr.length;
  //       i++) { document.getElementById('uline' + i).innerHTML =
  //       receivedArr[i]; } } }; xhttp.open('GET', 'data', true);
  //       xhttp.send();} getData(); setInterval(function() { getData(); }, %d);
  //       </script> )=====", dataRefresh * 1000);

  // get a comma seperated list from http://x.x.x.x/data and populate the blank
  // lines in html above
  client.printf(R"=====(
         <script>
            function getData() {
  ...
  client.write("<br><br>");
  client.write(
      "<input style='height: 35px;' name='button1' value='Toggle pin 12' "
      "type='submit'> \n");
  client.write(
      "<input style='height: 35px;' name='button2' value='Cycle illumination "
      "LED' type='submit'> \n");
  client.write(
      "<input style='height: 35px;' name='button3' value='Toggle Flash' "
      "type='submit'> \n");
  client.write(
      "<input style='height: 35px;' name='button4' value='Wipe SPIFFS memory' "
      "type='submit'> \n");
  client.write(
      "<input style='height: 35px;' name='button5' value='Change Resolution' "
      "type='submit'><br> \n");

  // Image setting controls
  client.println("<br>CAMERA SETTINGS: ");
  client.printf(
      "Brightness: <input type='number' style='width: 50px' name='bright' "
      "title='from -2 to +2' min='-2' max='2' value='%d'>  \n",
      cameraImageBrightness);
  client.printf(
      "Exposure: <input type='number' style='width: 50px' name='exp' "
      "title='from 0 to 1200' min='0' max='1200' value='%d'>  \n",
      cameraImageExposure);
  client.printf(
      "Gain: <input type='number' style='width: 50px' name='gain' title='from "
      "0 to 30' min='0' max='30' value='%d'>\n",
      cameraImageGain);
  client.println(
      " <input type='submit' name='submit' value='Submit change / Refresh "
      "Image'>");
  ...
  if (sdcardPresent) {
    client.write(
        "<br>Note: You can view the individual stored images on sd card with:  "
        " http://x.x.x.x/img?img=1");
  }

  // capture and show a jpg image
  client.write("<br><a href='/jpg'>");  // make it a link
  client.write(
      "<img id='image1' src='/jpg' width='320' height='240' /> </a>");  // show
                                                                        // image
                                                                        // from
                                                                        // http://x.x.x.x/jpg
  ...
  client.printf(R"=====(
         <script>
           ...
           setInterval(function() { refreshImage(); }, %d);
                ...
                imagerefresh * 1013);  // 1013 is just to stop it refreshing at
                                       // the same time as /data

  client.println(
      "<br><br><a href='https://github.com/alanesq/esp32cam-demo'>Sketch "
      "Info</a>");
...
// ----------------------------------------------------------------
// the root web page requests this periodically via Javascript in order to
// display updating information. information in the form of comma seperated text
// is supplied which are then inserted in to blank lines on the web page This
// makes it very easy to modify the data shown without having to change the
// javascript or root page html Note: to change the number of lines displayed
// update variable 'noLines' in handleroot()
...
void handleData() {
  ...
  if (!sdcardPresent) {
    ...
    reply += "SD Card: " + String(SDusedSpace) + "MB used - " +
             String(SDfreeSpace) + "MB free";
  }
  reply += ",";
  ...
  server.send(200, "text/plane",
              reply);  // Send millis value only to client ajax request
}
...
void handlePhoto() {
  ...
  byte sRes = storeImage();  // capture and save an image to sd card or spiffs
                             // (store sucess or failed flag - 0=fail, 1=spiffs
                             // only, 2=spiffs and sd card)
...
bool handleImg() {
  ...
  IPAddress cIP = client.remoteIP();
  if (serialDebug)
    Serial.println("Display stored image requested by " + cIP.toString());
  ...
  if (server.hasArg("img") && sdcardPresent) {
    ...
    if (imgToShow < 1 || imgToShow > imageCounter)
      imgToShow = imageCounter;  // validate image number
  }

  // if stored on sd card
  if (sdcardPresent) {
    if (serialDebug)
      Serial.printf("Displaying image #%d from sd card", imgToShow);
  ...
  if (!sdcardPresent) {
    ...
    if (!f) {
      ...
      size_t sent =
          server.streamFile(f, "image/jpeg");  // send file to web page
      if (!sent) {
...
// Notes:
//     Set sendRGBfile to 1 in the settings at top of sketch to just send the
//     raw rgb data as a file which can then be used with
...
//     You may want to disable auto white balance when experimenting with RGB
//     otherwise the camera is always trying to adjust the
//        image colours to mainly white.   (disable in the 'cameraImageSettings'
//        procedure).
//     It will fail on the higher resolutions as it requires more than the 4mb
//     of available psram to store the data (1600x1200x3 bytes) I learned how to
//     read the RGB data from:
//     https://github.com/Makerfabs/Project_Touch-Screen-Camera/blob/master/Camera_v2/Camera_v2.ino
...
void readRGBImage() {
  ...
  if (!sendRGBfile) {
    ...
    IPAddress cIP = client.remoteIP();
    sendText(client, "Live image as rgb data, requested by " +
                         cIP.toString());  // 'sendText' sends the String to
                                           // both serial port and web page
  }
  ...
  if (!fb) {
    ...
    sendText(client,
             "JPG image capture took " + String(millis() - tTimer) +
                 " milliseconds");  // report time it took to capture an image
    sendText(client, "Image resolution=" + String(fb->width) + "x" +
                         String(fb->height));
  ...
  sendText(client,
           "<br>Free psram before rgb data allocated = " +
               String(heap_caps_get_free_size(MALLOC_CAP_SPIRAM) / 1024) + "K");
  void *ptrVal =
      NULL;  // create a pointer for memory location to store the data
  uint32_t ARRAY_LENGTH = fb->width * fb->height *
                          3;  // calculate memory required to store the RGB data
                              // (i.e. number of pixels in the jpg image x 3)
  ...
  sendText(client,
           "Free psram after rgb data allocated = " +
               String(heap_caps_get_free_size(MALLOC_CAP_SPIRAM) / 1024) + "K");
  ...
  sendText(client, "Conversion from jpg to RGB took " +
                       String(millis() - tTimer) +
                       " milliseconds");  // report how long the conversion took
  ...
  sendText(client, "<br>R,G,B data for first " + String(resultsToShow / 3) +
                       " pixels of image");
  for (uint32_t i = 0; i < resultsToShow - 2; i += 3) {
    sendText(client, String(rgb[i + 2]) + "," + String(rgb[i + 1]) + "," +
                         String(rgb[i + 0]));  // Red , Green , Blue
  ...
  uint32_t aGreen = 0;
  uint32_t aBlue = 0;
  for (uint32_t i = 0; i < (ARRAY_LENGTH - 2);
       i += 3) {  // go through all data and add up totals
    aBlue += rgb[i];
    aGreen += rgb[i + 1];
    aRed += rgb[i + 2];
  }
  aRed = aRed / (fb->width * fb->height);  // divide total by number of pixels
  ...
  sendText(client, "Image luminance = " +
                       String(aRed * 0.3 + aGreen * 0.59 + aBlue * 0.11));
...
// ----------------------------------------------------------------
// Sends video stream - thanks to Uwe Gerlach for the code showing me how to do
// this
...
void handleStream() {
  ...
  const char BOUNDARY[] =
      "\r\n--123456789000000000000987654321\r\n";  // marks end of each image
                                                   // frame
  const char CTNTTYPE[] =
      "Content-Type: image/jpeg\r\nContent-Length: ";  // marks start of image
                                                       // data
  const int hdrLen = strlen(
      HEADER);  // length of the stored text, used when sending to web page
  ...
  while (true) {
    ...
    if (!fb) {
      ...
      // send image
      client.write(CTNTTYPE,
                   cntLen);  // send content type html (i.e. jpg image)
      sprintf(buf, "%d\r\n\r\n",
      ...
      client.write(
          BOUNDARY,
          bdrLen);  // send html boundary      see
                    // https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Type
      esp_camera_fb_return(
          fb);  // return image buffer so memory can be released
    }
  }
...
int requestWebPage(String *page, String *received, int maxWaitTime = 5000) {
  ...
  http.begin(
      client,
      *page);  // for https requires (client, *page, thumbprint)  e.g.String
               // thumbprint="08:3B:71:72:02:43:6E:CA:ED:42:86:93:BA:7E:DF:81:C4:BC:62:30";
  int httpCode =
      http.GET();  // http codes:
...
void handleJpeg() {
  ...
  // capture and show a jpg image
  client.write(
      "<img id='image1' src='/jpg'/>");  // show image from http://x.x.x.x/jpg
...
void handleTest() {
  ...
  client.println("var maxIndex = " + String(maxIndex) + ";");
  client.print(R"=====(
  ...
  /*
   // demo of how to request a web page
     ...
     // show results
       client.println("Web page requested: '" + page + "' - http code: " +
   String(httpCode)); client.print("<xmp>'");     // enables the html code to be
   displayed client.print(response); client.println("'</xmp><br>");
  */

  /*
  //  // demo useage of the mcp23017 io chipnote: this stops PWM on the flash
  working for some reason #if useMCP23017 == 1 while(1) { mcp.digitalWrite(0,
  HIGH); int q = mcp.digitalRead(8); client.print("<p>HIGH, input =" + String(q)
  + "</p>"); delay(1000); mcp.digitalWrite(0, LOW); client.print("<p>LOW</p>");
            delay(1000);
        }
      #endif
  */
